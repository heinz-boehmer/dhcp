...
// NTPSuboptions is a collection of NTPSuboption.
type NTPSuboptions Options
...
func parseNTPSuboption(code OptionCode, data []byte) (Option, error) {
	...
	case NTPSuboptionSrvFQDNCode:
		...
		return &so, nil
	default:
		return nil, fmt.Errorf("unknown NTP sub-option code %d", code)
	}
}
...
func ParseOptNTPServer(data []byte) (*OptNTPServer, error) {
	...
	return &OptNTPServer{
		Suboptions: NTPSuboptions(so),
	}, nil
}

// FromBytes parses a sequence of bytes into an NTPSuboptions object.
func (o *NTPSuboptions) FromBytes(data []byte) error {
	oo := Options(*o)
	return oo.FromBytesWithParser(data, parseNTPSuboption)
}

// OptNTPServer is an option NTP server as defined by RFC 5908.
type OptNTPServer struct {
	Suboptions NTPSuboptions
}
